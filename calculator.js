(() => {
  ...
  function calculateExpression(expression) {
    const calculatedNumber = methods[expression.operator](
  ...
  btnContainer.addEventListener("click", (event) => {
    ...
    switch (target.className.split(" ")[0]) {
      ...
      case "backspace":
        if (expression.rightOperand === null) {
          if (
            expression.leftOperand.length === 1 ||
            (expression.leftOperand.length === 2 &&
              expression.leftOperand[0] === "-")
          ) {
            expression.leftOperand = "0";
            break;
        ...
        if (
          expression.rightOperand.length === 1 ||
          (expression.rightOperand.length === 2 &&
      ...
      case "operator":
        if (expression.operator === null || expression.rightOperand === null) {
          if (expression.operatorElement) {
            expression.operatorElement.classList.remove("current-operator");
          }
    ...
    if (expression.rightOperand === null) {
      if (expression.leftOperand.length > MAX_LENGTH + 1) {
        return (display.textContent = (+expression.leftOperand).toExponential(
          2
        ));
      }
    ...
    if (expression.rightOperand.length > MAX_LENGTH + 1) {
      return (display.textContent = (+expression.rightOperand).toExponential(
        2
      ));
    }
